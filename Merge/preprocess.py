...
def build_save_in_shards(src_corpus, tgt_corpus, posi_neg_indicators, fields,
                         corpus_type, opt):
    """
    ...
    # add for reranker
    posi_neg_indicators_iter = inputters.ShardedTextCorpusIterator(
        posi_neg_indicators, 0,
        "posi_neg_indicators", opt.max_shard_size,
        assoc_iter=src_iter)
    ...
    while not src_iter.hit_end():
        ...
        dataset = inputters.TextDataset(
            fields, src_iter, posi_neg_indicators_iter, tgt_iter,
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        # add for reranker
        posi_neg_indicators = opt.train_posi_neg_indicators
    else:
        src_corpus = opt.valid_src
        tgt_corpus = opt.valid_tgt
        # add for reranker
        posi_neg_indicators = opt.valid_posi_neg_indicators

    # Currently we only do preprocess sharding for corpus: data_type=='text'.
    if opt.data_type == 'text':
        return build_save_in_shards(
            src_corpus, tgt_corpus, posi_neg_indicators, fields,
            corpus_type, opt)
