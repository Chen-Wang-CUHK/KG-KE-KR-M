...
from onmt.reranker.rnn_reranker import RNNReRanker

from onmt.decoders.decoder import InputFeedRNNDecoder, StdRNNDecoder, MyInputFeedRNNDecoder
...
def build_reranker(opt, embeddings):
    """
    ...
    """
    return RNNReRanker(opt.rnn_type, opt.brnn, opt.enc_layers,
                       opt.rnn_size, opt.dropout, embeddings)


def build_selector(opt, embeddings):
    """
    Build the selector network.
    Args:
        opt: the option in current environment.
...
def build_end2end_model(model_opt, fields, gpu, checkpoint=None, sel_checkpoint=None, s2s_gen_checkpoint=None):
    ...
    device = torch.device("cuda" if gpu else "cpu")
    model = onmt.models.E2EModel(encoder, selector, decoder,
