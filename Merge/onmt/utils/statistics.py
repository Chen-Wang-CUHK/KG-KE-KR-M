...
class E2EStatistics(object):
    ...
    def ave_total_norm(self):
        if len(self.batch_size_list) == 0:
            return 0.0
        else:
            return self.total_norm / len(self.batch_size_list)
    ...
    def update(self, stat):
        ...
        # for total norm
        self.total_norm += stat.total_norm
    ...
    def log_tensorboard(self, prefix, writer, learning_rate, step):
        ...
        writer.add_scalar(prefix + "/total_norm", self.ave_total_norm(), step)
...
class ReRankerStatistics(object):
    """
    Accumulator for loss statistics of the reranker.
    """

    def __init__(self, batch_size=0, loss=0, posi_gt=0, neg_gt=0, posi_pred_correct=0, neg_pred_correct=0):
        self.loss = loss
        self.loss_list = [] if loss == 0 else [loss]
        self.batch_size_list = [] if batch_size == 0 else [batch_size]
        self.posi_gt = posi_gt
        self.neg_gt = neg_gt
        self.posi_pred_correct = posi_pred_correct
        self.neg_pred_correct = neg_pred_correct
    ...
    def posi_acc(self):
        """ compute the accuracy of the posi prediction """
        return self.posi_pred_correct * 1.0 / self.posi_gt

    def neg_acc(self):
        """ compute the accuracy of the neg prediction """
        return self.neg_pred_correct * 1.0 / self.neg_gt

    def total_acc(self):
        """ compute the accuracy of the posi & neg prediction """
        return (self.neg_pred_correct + self.posi_pred_correct) * 1.0 / (self.neg_gt + self.posi_gt)
    ...
    def update(self, stat):
        ...
        self.batch_size_list += stat.batch_size_list
        self.posi_gt += stat.posi_gt
        self.neg_gt += stat.neg_gt
        self.posi_pred_correct += stat.posi_pred_correct
        self.neg_pred_correct += stat.neg_pred_correct
        self.total_norm += stat.total_norm
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        t = self.elapsed_time()
        logger.info(
            ("Step %06d; ave_loss: %7.5f; posi_acc: %4.2f; neg_acc: %4.2f; total_acc: %4.2f; " +
             "lr: %7.5f; tnorm: %4.2f; %6.0f sec")
            % (step, self.ave_loss(),
               self.posi_acc(),
               self.neg_acc(),
    ...
    def log_tensorboard(self, prefix, writer, learning_rate, step):
        """ display statistics to tensorboard """
        # reranker
        writer.add_scalar(prefix + "/ave_loss", self.ave_loss(), step)
        writer.add_scalar(prefix + "/posi_acc", self.posi_acc(), step)
        writer.add_scalar(prefix + "/neg_acc", self.neg_acc(), step)
        writer.add_scalar(prefix + "/total_acc", self.total_acc(), step)
        # public
        writer.add_scalar(prefix + "/lr_rate", learning_rate, step)
        writer.add_scalar(prefix + "/total_norm", self.ave_total_norm(), step)
