...
class MyReportMgr(ReportMgrBase):
    ...
    def report_training(self, step, num_steps, learning_rate,
        ...
        if step % self.report_every == 0:
            self._report_training(
                step, num_steps, learning_rate, report_stats)
            self.progress_step += 1
            return onmt.utils.ReRankerStatistics()
        else:
            return report_stats
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if train_stats is not None:
            self.log(' ')
            self.log('ReRanker Train ave_loss: %g' % train_stats.ave_loss())
            self.log('ReRanker Train posi_acc: %g' % train_stats.posi_acc())
            self.log('ReRanker Train neg_acc: %g' % train_stats.neg_acc())
            self.log('ReRanker Train total_acc: %g' % train_stats.total_acc())
            self.maybe_log_tensorboard(train_stats, "train", lr, step)

            self.log('==========================================================================')

        if valid_stats is not None:
            self.log('ReRanker Valid ave_loss: %g' % valid_stats.ave_loss())
            self.log('ReRanker Valid posi_acc: %g' % valid_stats.posi_acc())
            self.log('ReRanker Valid neg_acc: %g' % valid_stats.neg_acc())
            self.log('ReRanker Valid total_acc: %g' % valid_stats.total_acc())
