...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        self.attn = onmt.modules.GlobalAttention(
            ...
            attn_type=attn_type, no_sftmax_bf_rescale=no_sftmax_bf_rescale)

        # Set up a separated copy attention layer, if needed.
        self._copy = False
        if copy_attn and not reuse_copy_attn:
            self.copy_attn = onmt.modules.GlobalAttention(
                hidden_size, attn_type=attn_type, no_sftmax_bf_rescale=no_sftmax_bf_rescale)
        if copy_attn:
            self._copy = True
        self._reuse_copy_attn = reuse_copy_attn

    def forward(self, tgt, memory_bank, state, memory_lengths=None, probs=None,
                step=None):
        """
        Args:
        ...
        decoder_final, decoder_outputs, attns = self._run_forward_pass(
            tgt, memory_bank, state, memory_lengths=memory_lengths, probs=probs)
...
class MyInputFeedRNNDecoder(RNNDecoderBase):
    """
    Input feeding based decoder. See :obj:`RNNDecoderBase` for options.
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 coverage_attn=False, context_gate=None,
                 copy_attn=False, dropout=0.0, embeddings=None,
                 reuse_copy_attn=False, not_use_sel_probs=False, no_sftmax_bf_rescale=False):
