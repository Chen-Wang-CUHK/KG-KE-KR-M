...
def model_opts(parser):
    ...
    group.add_argument('-key_model', type=str, default='key_generator',
                       ...
                       help="""Type of model to use. Options are
                               [key_selector|key_generator|end2end|reranker].""")
    group.add_argument('-not_use_sel_probs', action='store_true',
                       help="""Do not use the selection probs to renormalize the attention score""")
    group.add_argument('-no_sftmax_bf_rescale', action='store_true',
                       help="""Do not use softmax to normalize the attn scores and instead use the sigmoid function + summation normalization""")
    group.add_argument('-use_gt_sel_probs', action='store_true',
                       help="""Use the ground-truth of the selector probabilities as the output of the selector""")
    # Nallapati
    ...
    group.add_argument('-e2e_type', type=str, default='separate_enc_sel',
                       ...
                       help="""Whether share the BiRNN and embedding layers of the encoder and selector.
...
def preprocess_opts(parser):
    ...
    # add for keyword extraction KE_KG reranker
    group.add_argument('-train_posi_neg_indicators', required=True,
                       help="Path to the training posi_neg_indicators data")
    ...
    # add for keyword extraction KE_KG reranker
    group.add_argument('-valid_posi_neg_indicators', required=True,
                       help="Path to the valid posi_neg_indicators data")
...
def train_opts(parser):
    ...
    # add for KE_KG load_pretrained_selector_from
    ...
    # changed for KE_KG ["sents", "tokens"] -> ["sents"]
...
def translate_opts(parser):
    ...
    group.add_argument('-gen_scores', default='pred_scores.txt',
                       help="""Path to the scores of the generator's predictions""")
    group.add_argument('-sel_probs',
                       help="""Path to the extraction scores of the extractor's predictions""")
    group.add_argument('-sel_keys_output', default='sel_keys.txt',
                       help="""Path to the extracted keyphrases by the extractor""")
    group.add_argument('-reranked_scores_output', default='reranked_scores.txt',
                       help="""Path to output the predicted scores by the reranker""")
    group.add_argument('-retrieved_keys',
                       help="""Path to the retrieved keyphrases""")
    group.add_argument('-retrieved_scores',
                       help="""Path to the retrieval scores of the retrieved keyphrases""")
    group.add_argument('-output', default='reranked_output.txt',
                       help="""Path to output the reranked keyphrases""")
    group.add_argument('-merge_ex_keys', action="store_true")
    group.add_argument('-merge_rk_keys', action="store_true")
    group.add_argument('-merge_with_stemmer', action="store_true")
    ...
    # add for DKG evaluations
    group.add_argument('-tmp_folder', type=str, default='tmp',
                       help="""The temporal folder""")
    group.add_argument('-posi_neg_indicator',
                       help="""True posi_neg_indicator sequence""")
    parser.add_argument('-single_word_maxnum', type=int, default=-1)
    parser.add_argument('-filter_dot_comma_unk', type=bool, default=False)
    parser.add_argument('-match_method', type=str, default='str_match',
                        choices=['str_match', 'word_match'])
    parser.add_argument('-kpg_context', type=str, required=True)
    parser.add_argument('-kpg_tgt', type=str, required=True)
    parser.add_argument('-stopwords_file', type=str,
                        default='data\\stfd_stopwords\\corenlp_stopwords.json')
...
def merge_opts(parser):
    """ merge and rerank options """
    group = parser.add_argument_group('Model')
    group.add_argument('-gpu', type=int, default=-1,
                       help="Device to run on")
    group.add_argument('-model', required=True,
                       help='Path to score model .pt file')
    group = parser.add_argument_group('Data')
    group.add_argument('-data_type', default="text",
                       help="Type of the source input. Options: [text|img].")
